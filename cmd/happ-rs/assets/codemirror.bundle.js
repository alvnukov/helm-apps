(() => {
  ...
  function foldByIndent(view, level) {
    ...
    for (let i = 1; i <= doc2.lines; i += 1) {
      ...
      let nextLine = i + 1;
      while (nextLine <= doc2.lines) {
        const n = doc2.line(nextLine);
        if (n.text.trim()) break;
        nextLine += 1;
      }
      if (nextLine > doc2.lines) continue;
      const nline = doc2.line(nextLine);
      ...
      let end = nextLine;
      for (let j = nextLine + 1; j <= doc2.lines; j += 1) {
        const test = doc2.line(j);
        if (!test.text.trim()) {
          end = j;
          continue;
        }
  ...
  var jetBrainsLikeHighlight = HighlightStyle.define([
    { tag: [tags.propertyName, tags.attributeName], color: "#d19a66" },
    { tag: [tags.keyword, tags.operatorKeyword], color: "#cc7832" },
    { tag: [tags.string, tags.special(tags.string)], color: "#98c379" },
    { tag: [tags.number], color: "#d19a66" },
    { tag: [tags.bool, tags.null], color: "#c678dd" },
    { tag: [tags.comment], color: "#6a8f74", fontStyle: "italic" },
    { tag: [tags.variableName, tags.typeName, tags.className], color: "#c8c8c8" },
