...
`helm-apps` это библиотека Helm-шаблонов для стандартизированного и предсказуемого деплоя приложений в Kubernetes.
Вместо копирования шаблонов между сервисами команда описывает приложение через единый `values.yaml`-контракт.

Библиотека полностью поддерживает `Helm` и совместима с `werf`.  
Под капотом используются helper-паттерны `flant-lib` (`fl.*`) как базовый слой обработки значений и рендера:  
...
|---|---|
| Разработчики | Единый формат `values.yaml`, меньше ручного Kubernetes YAML, быстрее onboarding |
